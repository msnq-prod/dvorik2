...
try {
  await new Promise<void>((resolve, reject) => { const timeout = setTimeout(() => reject(new Error(`Staff readiness timeout: ${output}`)), 15_000); const ready = () => { if (output.includes("Dvorik Staff:")) { clearTimeout(timeout); resolve(); } }; child.stdout.on("data", ready); child.once("exit", (code, signal) => { clearTimeout(timeout); reject(new Error(`Staff exited code=${code} signal=${signal}: ${output}`)); }); });
  const readiness = await (await fetch(`http://127.0.0.1:${port}/ready`)).json() as { ready: boolean; outbox: { failedEvents: number } };
  assert.equal(readiness.ready, true);
  assert.equal(readiness.outbox.failedEvents, 0);
