...
assert.deepEqual(routePolicyFor("POST", "/api/labels/jobs"), { permission: "labels:print" });
assert.deepEqual(routePolicyFor("POST", "/api/labels/jobs/job-1/reprint"), { permission: "labels:print" });
...
assert.deepEqual(routePolicyFor("POST", "/api/products/quick-scan"), { permission: "products:scan_manage" });
assert.deepEqual(routePolicyFor("POST", "/api/products/p-1/barcodes"), { permission: "products:scan_manage" });
assert.deepEqual(routePolicyFor("POST", "/api/warehouse/write-offs"), { permission: "stock:move" });
assert.deepEqual(routePolicyFor("POST", "/api/warehouse/adjustments"), { permission: "stock:move" });
assert.deepEqual(routePolicyFor("POST", "/api/warehouse/supply-previews"), { permission: "stock:move" });
assert.deepEqual(routePolicyFor("POST", "/api/warehouse/products"), { permission: "products:write" });
