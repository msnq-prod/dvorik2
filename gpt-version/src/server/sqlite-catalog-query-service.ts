import type { AuditEntry, InventoryBalance, InventorySnapshotRow, Location, Manufacturer, MovementReportRow, NotificationItem, Product, ProductGroup, ProductPackaging, ProductPriceHistory, StockBalance, StockOperation } from "../shared/types";
...
export class SqlCatalogQueryService {
  ...
  productByBarcode(barcode: string): Product | undefined {
    const row = this.database.query<Row>(
      `SELECT p.*
       FROM products p
       JOIN product_identifiers identifier ON identifier.product_id = p.id
       WHERE identifier.type = 'barcode' AND identifier.normalized_value = ?
       ORDER BY p.id
       LIMIT 1`,
      [barcode.trim().toLocaleLowerCase("ru-RU")]
    )[0];
    return row ? productFromRow(this.database, row) : undefined;
  }
