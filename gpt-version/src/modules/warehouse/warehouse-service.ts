...
export class WarehouseService {
  constructor(private readonly database: DatabaseAdapter, private readonly now: () => string = () => new Date().toISOString()) {
    // Warehouse owns command replay protection. This is deliberately separate
    // from Core idempotency because the services have independent databases.
    this.database.executeScript(`CREATE TABLE IF NOT EXISTS warehouse_idempotency_keys(
      scope TEXT NOT NULL, key TEXT NOT NULL, request_json TEXT NOT NULL, response_json TEXT NOT NULL, created_at TEXT NOT NULL,
